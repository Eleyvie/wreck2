...
# Colorama library is now only required for colored output on Windows. On other systems missing colorama library will not result in fallback to black&white mode.
...
# |
    ...
    try:
        for entity_name, entity_def in parsers.iteritems():
            ...
            globals()[entity_name] = check_syntax(globals()[entity_name], [entity_def['parser']], entity_def.get('uid', 0))
        WRECK.current_module = None
    except Exception as e:
    ...
    with WRECK.anim as module_data: module_data.source = animations
    with WRECK.fac as module_data: module_data.source = factions
    with WRECK.ip as module_data: module_data.source = info_pages
    with WRECK.imod as module_data: module_data.source = item_modifiers
    with WRECK.itm as module_data: module_data.source = items
    with WRECK.icon as module_data: module_data.source = map_icons
    with WRECK.mnu as module_data: module_data.source = game_menus
    with WRECK.mesh as module_data: module_data.source = meshes
    with WRECK.mt as module_data: module_data.source = mission_templates
    with WRECK.track as module_data: module_data.source = tracks
    with WRECK.psys as module_data: module_data.source = particle_systems
    with WRECK.p as module_data: module_data.source = parties
    with WRECK.pt as module_data: module_data.source = party_templates
    with WRECK.pfx as module_data: module_data.source = postfx_params
    with WRECK.prsnt as module_data: module_data.source = presentations
    with WRECK.qst as module_data: module_data.source = quests
    with WRECK.spr as module_data: module_data.source = scene_props
    with WRECK.scn as module_data: module_data.source = scenes
    with WRECK.script as module_data: module_data.source = scripts
    with WRECK.skl as module_data: module_data.source = skills
    with WRECK.snd as module_data: module_data.source = sounds
    with WRECK.s as module_data: module_data.source = strings
    with WRECK.tableau as module_data: module_data.source = tableaus
    with WRECK.trp as module_data: module_data.source = troops
    ...
    if write_id_files is not None:
        ...
        try:
            for entity_name, (entity, prefix) in export.iteritems():
                contents = '\n'.join(['%s%s = %d' % (prefix, ref, index) for ref, index in sorted(map(lambda i:(i[0],int(i[1]&0xFFFFFFFF)), iter(entity)), lambda x,y:cmp(x[1],y[1]))])
