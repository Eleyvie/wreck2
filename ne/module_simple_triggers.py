...
#    (start_presentation, "prsnt_custom_banner"),
  ...
  # NE kingdom
    # Added kingdom morale here to not break saves.
  # Ne end 
  (24,
   [
...
# Kingdom morale begins here - Jinnai
# Doubling influence from 3000 to 6000 - soulmata
      (eq,"$setting_towns_rebel",1),
      (faction_slot_eq, "fac_player_supporters_faction", slot_faction_state, sfs_active),
      (faction_slot_eq, "fac_player_supporters_faction", slot_faction_leader, "trp_player"),
      (assign, ":castles_owned", 0),
      (assign, ":towns_owned", 0),
      (try_for_range, ":center_no", towns_begin, towns_end),
        (party_slot_eq, ":center_no", slot_town_lord, "trp_player"),
        (val_add,":towns_owned",1),
      (try_end),
      (try_for_range, ":center_no", castles_begin, castles_end),
        (party_slot_eq, ":center_no", slot_town_lord, "trp_player"),
        (val_add,":castles_owned",1),
      (try_end),
      (this_or_next|gt,":towns_owned",0),
      (gt,":castles_owned",1), #Allow the kingdom to have one castle without needing a capital
      (assign,":radius",6000),
      #(try_begin),
      #  (neg|party_slot_eq,":capital",slot_town_lord,"trp_player"),
      #  (assign,":capital","p_main_party"),
      #  (assign,":radius",6000),
      #(else_try),
      #  (le,":capital",0),
      #  (assign,":capital","p_main_party"),
      ...
      (try_for_range,":center_no",centers_begin, centers_end),
        ...
        (lt,":relation",20), #only degrade if the relation is less than 20
        (val_add,":relation",-1), #Subtract 1 relation from all player owned towns outside 4000 of the capital city
        (val_clamp, ":relation", -100, 100),
        (party_set_slot, ":center_no", slot_center_player_relation, ":relation"),
        (try_begin),
          (le,":relation",-40), #Check for rebellion
          (call_script,"script_give_center_to_faction", ":center_no", "fac_outlaws"),
          (try_begin),
            ...
            (try_end),
            (party_set_slot, ":center_no", slot_village_infested_by_bandits, ":bandit_troop"),
          (try_end),
          (str_store_party_name,s1,":center_no"),
          (display_message,"@{s1} has rebelled from your kingdom!",0xFF2222),
          (call_script, "script_add_notification_menu", "mnu_notification_rebellion", ":center_no", 0),
        (else_try),
          (le,":relation",-30), #Give a warning to the player if they have a civil adviser
          (faction_slot_eq, "fac_player_supporters_faction", slot_faction_civil_adviser, 1),
          (str_store_party_name,s1,":center_no"),
          (str_store_troop_name,s2,"trp_civil_adviser"),
          (display_message,"@Your {s2} warns you that {s1} is close to rebellion!",0xFF2222),
        (try_end),
      (try_end),
...
#Party AI: pruning some of the prisoners in each center (once a week)
  (24*7,
   [
    ...
    # NE rebellion
#Lord rebellion stuff begins here - Jinnai
      (eq,"$setting_lords_rebel",1),
      (faction_slot_eq, "fac_player_supporters_faction", slot_faction_state, sfs_active),
      (faction_slot_eq, "fac_player_supporters_faction", slot_faction_leader, "trp_player"),
      (try_for_range,":lord_no",active_npcs_begin,active_npcs_end),
        ...
        (gt,":lord_party",0), # Only allow lords that have active parties to start a rebellion
        ...
        (try_begin),
          ...
          (eq,":rep",lrep_goodnatured),
          (assign,":rebel_limit",-60),
          (assign,":lower_limit",15),
        (else_try),
          (this_or_next|eq,":rep",lrep_none),
          (eq,":rep",lrep_martial),
          (assign,":rebel_limit",-40),
          (assign,":lower_limit",25),
        (else_try),
#          (this_or_next|eq,":rep",lrep_quarrelsome),
#          (this_or_next|eq,":rep",lrep_selfrighteous),
#          (this_or_next|eq,":rep",lrep_cunning),
#          (eq,":rep",lrep_debauched),
          (assign,":rebel_limit",-20),
          (assign,":lower_limit",35),
        (try_end),
        (try_begin),
          (troop_slot_eq, ":lord_no", slot_troop_no_rebellion, 1),
          (troop_set_slot, ":lord_no", slot_troop_no_rebellion, 0),
        (else_try),
          (ge,":relation",":lower_limit"),
          (troop_set_slot, ":lord_no", slot_troop_no_rebellion, 0),
        (else_try),
          (troop_set_slot, ":lord_no", slot_troop_no_rebellion, 0),
          (val_add,":relation",-5),
          (val_clamp, ":relation", -100, 100),
          (troop_set_slot, ":lord_no", slot_troop_player_relation, ":relation"),
          (le,":relation",":rebel_limit"), # We have a rebellion starting
          (troop_get_slot,":old_faction",":lord_no",slot_troop_original_faction),
          (assign,":reform",0),
          (try_begin),
            (neg|faction_slot_eq, ":old_faction", slot_faction_state, sfs_active),
            (assign,":reform",1),
            (call_script,"script_change_troop_faction", ":lord_no", ":old_faction"),
            (faction_get_slot,":monarch",":old_faction",slot_faction_leader),
            (troop_get_slot, ":town", ":monarch", slot_troop_prisoner_of_party),
            (try_begin),
              (ge,":town",0),
              (party_remove_prisoners, ":town", ":monarch", 1),
              (call_script, "script_remove_troop_from_prison", ":monarch"),
              (str_store_troop_name_link,s1,":lord_no"),
              (str_store_faction_name_link,s2,":old_faction"),
              (str_store_troop_name_link,s3,":monarch"),
              (display_message,"@{s1} has rebelled, freed {s3} from captivity, and reformed the {s2}!",0xFF2222),
              (call_script, "script_add_notification_menu", "mnu_notification_lord_restart_freed", ":lord_no", ":town"),
            (else_try),
              (call_script,"script_change_troop_faction", ":monarch", ":old_faction"),
              (str_store_troop_name_link,s1,":lord_no"),
              (str_store_faction_name_link,s2,":old_faction"),
              (str_store_troop_name_link,s3,":monarch"),
              (display_message,"@{s1} has rebelled and reformed the {s2} with {s3}!",0xFF2222),
              (call_script, "script_add_notification_menu", "mnu_notification_lord_restart", ":lord_no", ":monarch"),
            (try_end),
          (else_try),
            (call_script,"script_change_troop_faction", ":lord_no", ":old_faction"),
            (str_store_troop_name_link,s1,":lord_no"),
            (str_store_faction_name_link,s2,":old_faction"),
            (display_message,"@{s1} has rebelled and rejoined the {s2}!",0xFF2222),
            (call_script, "script_add_notification_menu", "mnu_notification_lord_rebellion", ":lord_no", 0),
          (try_end),
          (eq,":reform",1), #Only let other lords join if the faction was completely restarted
          (try_for_range,":troop_no",active_npcs_begin,active_npcs_end),
            (store_troop_faction, ":faction", ":troop_no"),
            (troop_get_slot,":return_faction",":troop_no",slot_troop_original_faction),
            (eq,":old_faction",":return_faction"),
            (try_begin),
              (eq,":faction","fac_player_supporters_faction"),
              (troop_get_slot, ":rep", ":troop_no", slot_lord_reputation_type),
              (call_script, "script_troop_get_player_relation",":troop_no"),
              (assign, ":relation", reg0),
              (try_begin),
                (this_or_next|eq,":rep",lrep_upstanding),
                (eq,":rep",lrep_goodnatured),
                (assign,":join_limit",-30),
              (else_try),
                (this_or_next|eq,":rep",lrep_none),
                (eq,":rep",lrep_martial),
                (assign,":join_limit",-10),
              (else_try),
                (assign,":join_limit",5),
              (try_end),
              (le,":relation",":join_limit"),
              (call_script,"script_change_troop_faction", ":troop_no", ":old_faction"),
              (str_store_faction_name_link,s3,":faction"),
              (str_store_troop_name_link,s1,":troop_no"),
              (str_store_faction_name_link,s2,":old_faction"),
              (display_message,"@{s1} has joined the rebellion and left the {s3} and rejoined the {s2}!",0xFF2222),
              (call_script, "script_add_notification_menu", "mnu_notification_lord_rebellion", ":troop_no", ":lord_no"),
            (else_try),
              (neq,":faction",":old_faction"),
              (neq,":faction","fac_player_supporters_faction"),
              (store_random_in_range,":rand",1,101),
              (le,":rand",50), #50/50 chance old lords from other factions will join the rebellion
              (call_script,"script_change_troop_faction", ":troop_no", ":old_faction"),
              (str_store_faction_name_link,s3,":faction"),
              (str_store_troop_name_link,s1,":troop_no"),
              (str_store_faction_name_link,s2,":old_faction"),
              (display_message,"@{s1} has joined the rebellion and left the {s3} and rejoined the {s2}!"),
            (try_end),
          (try_end),
        (try_end),
      (try_end),    
    ...
    # NE recruitment - to enable player towns recruiting, change this. 
...
# NE building
## Now use some of the leftover money to build upgrades for their lands - Jinnai
         (troop_get_slot, ":cur_wealth", ":troop_no", slot_troop_wealth),
         (try_begin),
           (ge,":cur_wealth",1000),
           (store_random_in_range,":rand",1,4),
           (le,":rand",1), #1 in 3 chance of building
           (assign,":centers",0),
           (try_for_range,":center_no",centers_begin,centers_end),
           ...
           (try_end),
           (val_add,":centers",1),
           (store_random_in_range,":rand",1,":centers"),
           (assign,":centers",0),
           (assign,":result",-1),
           (try_for_range,":center_no",centers_begin,centers_end),
             (party_slot_eq,":center_no",slot_town_lord,":troop_no"),
           ...
           (try_end),
           (ge,":result",0),
           (party_get_slot,":type",":result",slot_party_type),
           (assign,":upgrades",0),
           (try_begin),
             (eq,":type",spt_village),
             (try_for_range,":upgrade_no",village_improvements_begin,village_improvements_end),
               (party_slot_eq, ":result", ":upgrade_no", 0),
               (val_add,":upgrades",1),
             (try_end),
           (else_try),
             (eq,":type",spt_town),
             (try_for_range,":upgrade_no",walled_center_improvements_begin,walled_center_improvements_end),
               (party_slot_eq, ":result", ":upgrade_no", 0),
               (val_add,":upgrades",1),
             (try_end),
           (else_try),
             (try_begin),
               (party_slot_eq, ":result", slot_center_has_messenger_post, 0),
               (val_add,":upgrades",1),
             (try_end),
             (try_begin),
               (party_slot_eq, ":result", slot_center_has_prisoner_tower, 0),
               (val_add,":upgrades",1),
             (try_end),
             (try_begin),
               (party_slot_eq, ":result", slot_center_has_blacksmith, 0),
               (val_add,":upgrades",1),
             (try_end),
           (try_end),
           (val_add,":upgrades",1),
           (store_random_in_range,":rand",1,":upgrades"),
           (assign,":upgrade",-1),
           (assign,":upgrades",0),
           (try_begin),
             (eq,":type",spt_village),
           ...
           (else_try),
             (try_begin),
               ...
               (val_add,":upgrades",1),
               (eq,":rand",":upgrades"),
               (assign,":upgrade",slot_center_has_messenger_post),
             (try_end),
             (try_begin),
               (party_slot_eq, ":result", slot_center_has_prisoner_tower, 0),
               (val_add,":upgrades",1),
               (eq,":rand",":upgrades"),
               (assign,":upgrade",slot_center_has_prisoner_tower),
             (try_end),
             (try_begin),
               (party_slot_eq, ":result", slot_center_has_blacksmith, 0),
               (val_add,":upgrades",1),
               (eq,":rand",":upgrades"),
               (assign,":upgrade",slot_center_has_blacksmith),
             (try_end),
           (try_end),
           (ge,":upgrade",0),
           (call_script, "script_get_improvement_details", ":upgrade"),
           ...
           (val_div, ":improvement_time", 3),
           (val_add, ":improvement_time", 3),
           (party_set_slot, ":result", slot_center_current_improvement, ":upgrade"),
           (store_current_hours, ":cur_hours"),
           (store_mul, ":hours_takes", ":improvement_time", 24),
           (val_add, ":hours_takes", ":cur_hours"),
           (party_set_slot, ":result", slot_center_improvement_end_hour, ":hours_takes"),
           (val_sub,":cur_wealth",1000), #All buildings cost NPCs only 1000 denars because it's much harder for them to get money.
           (troop_set_slot,":troop_no", slot_troop_wealth, ":cur_wealth"),
           (try_begin),
             (store_faction_of_party, ":faction", ":result"),
             (eq,":faction","fac_player_supporters_faction"),
             (str_store_troop_name,s1,":troop_no"),
             (str_store_party_name,s2,":result"),
             (display_message,"@{s1} has started construction of a {s50} at {s2}."),
           (try_end),
         (try_end),
...
# NE end building
     ...
     (try_for_range, ":center_no", walled_centers_begin, walled_centers_end),
## NE - Kingdom reinforcements start - Jinnai     
       # (neg|party_slot_eq, ":center_no", slot_town_lord, "trp_player"), #center does not belong to player.
       # (party_slot_ge, ":center_no", slot_town_lord, 1), #center belongs to someone.     
       (party_slot_ge, ":center_no", slot_town_lord, 0), #center belongs to someone.  
## NE Kingdom reinforcements end       
...
# NE other 
## Make sure kings always have elite guard - Jinnai
       (try_for_range, ":troop_no", kings_begin, kings_end),
         ...
         (party_slot_eq, ":cur_attached_party", slot_center_is_besieged_by, -1), #center not under siege
         (try_begin),
           (eq,":troop_no","trp_kingdom_1_lord"), # Swadia
           (assign,":troop_to_add_1","trp_swadian_paladin"),
           (assign,":troop_to_add_2","trp_swadian_priest"), # Pete - changed from elite ranger # soulmata - changed to priest
         (else_try),
           (eq,":troop_no","trp_kingdom_2_lord"), # Vaegir
# Wellen v585
#           (assign,":troop_to_add_1","trp_vaegir_ecataphract"),
           (assign,":troop_to_add_1","trp_vaegir_druzhina"),
           (assign,":troop_to_add_2","trp_vaegir_ivory_sentinel"),
# end Wellen
           (assign,":troop_to_add_2","trp_vaegir_lineman"),
         (else_try),
           (eq,":troop_no","trp_kingdom_3_lord"), # Khergit
           (assign,":troop_to_add_1","trp_khergit_guanren"),
           (assign,":troop_to_add_2","trp_khergit_keshik"),
         (else_try),
           (eq,":troop_no","trp_kingdom_4_lord"), # Nord
           (assign,":troop_to_add_1","trp_nord_einherjar"),
           (assign,":troop_to_add_2","trp_nord_valkyrie"),
         (else_try),
           (eq,":troop_no","trp_kingdom_5_lord"), # Rhodok
           (assign,":troop_to_add_1","trp_rhodok_arbalest"),
           (assign,":troop_to_add_2","trp_rhodok_spear_knight"),
         (else_try),
           (eq,":troop_no","trp_kingdom_6_lord"), # Sarranid
           (assign,":troop_to_add_1","trp_sarranid_zhayedan"),
           (assign,":troop_to_add_2","trp_sarranid_spahbod_serden"),
         (else_try),
           (eq,":troop_no","trp_dark_knight_lord"), # Dark Knights
           (assign,":troop_to_add_1","trp_blackguard_lord"),
           (assign,":troop_to_add_2","trp_dark_ranger"),
         (try_end),
         ...
         (try_for_range, ":troop_iterator", 0, ":num_stacks"),
           (party_stack_get_troop_id, ":cur_troop_id", ":party_no", ":troop_iterator"),
           (try_begin),
             (eq,":cur_troop_id",":troop_to_add_1"),
             (assign,":foundone",1),
             (party_stack_get_size,":num_troops",":party_no",":troop_iterator"),
             (lt,":num_troops",20),
             (store_sub,":num_troops",20,":num_troops"),
             (assign, ":addedone",":num_troops"),
             (party_add_members,":party_no",":troop_to_add_1",":num_troops"),
           (else_try),
             (eq,":cur_troop_id",":troop_to_add_2"),
             (assign,":foundtwo",1),
             (party_stack_get_size,":num_troops",":party_no",":troop_iterator"),
             (lt,":num_troops",20),
             (store_sub,":num_troops",20,":num_troops"),
             (assign, ":addedtwo",":num_troops"),
             (party_add_members,":party_no",":troop_to_add_2",":num_troops"),
           (try_end),
         (try_end),
         (try_begin),
           (eq,":foundone",0), # None were found, add a whole new stack
           (party_add_members,":party_no",":troop_to_add_1",20),
           (assign,":addedone",20),
         (try_end),
         (try_begin),
           (eq,":foundtwo",0), # None were found, add a whole new stack
           (party_add_members,":party_no",":troop_to_add_2",20),
           (assign,":addedtwo",20),
         (try_end),
         # Now we move the elite units to the top of the stack if any were added
         (this_or_next|gt,":addedone",0),
         (gt,":addedtwo",0),
         ...
         (try_for_range, ":troop_iterator", 0, ":num_stacks"),
           ...
           (try_begin),
             ...
             (neg|eq,":cur_troop_id",":troop_to_add_1"),
             (neg|eq,":cur_troop_id",":troop_to_add_2"),
             (party_stack_get_size,":num_troops",":party_no",":troop_iterator"),
             (party_add_members,"p_temp_party",":cur_troop_id",":num_troops"),
           (else_try),
             (party_stack_get_size,":num_troops",":party_no",":troop_iterator"),
             (party_add_members,"p_temp_party_2",":cur_troop_id",":num_troops"),
           (try_end),
         (try_end),
         ...
         (try_for_range, ":troop_iterator", 0, ":num_stacks"),
           ...
           (ge,":cur_troop_id",0),
           (party_stack_get_size,":num_troops","p_temp_party_2",":troop_iterator"),
           (party_add_members,":party_no",":cur_troop_id",":num_troops"),
         (try_end),
         (party_get_num_companion_stacks,":num_stacks","p_temp_party"),
         (try_for_range, ":troop_iterator", 0, ":num_stacks"),
           (party_stack_get_troop_id, ":cur_troop_id", "p_temp_party", ":troop_iterator"),
           (ge,":cur_troop_id",0),
           (party_stack_get_size,":num_troops","p_temp_party",":troop_iterator"),
           (party_add_members,":party_no",":cur_troop_id",":num_troops"),
         (try_end),
       (try_end),
...
# NE end other 
      ...
      #Sorting higher level troops to the top of stacks 
  (24,
   [
       ...
       (try_for_range, ":troop_no", active_npcs_begin, active_npcs_end),
         ...
         (party_clear, "p_temp_party"),
         (party_clear, "p_temp_party_2"),
         (party_clear, "p_temp_party_3"),
         (party_clear, "p_temp_party_4"),         
         (party_clear, "p_temp_party_5"),
         (party_get_num_companion_stacks,":num_stacks",":party_no"),
         
         
         # First sort out prisoners
           ...
           (party_add_members,"p_temp_party_5",":stack_troop",":stack_size"),         
           (try_end),
         ...
         (try_for_range, ":troop_iterator", 0, ":num_stacks"),
           ...
           (try_begin),
             (neg|troop_is_hero,":cur_troop_id"),
             (store_character_level, ":troop_level", ":cur_troop_id"),
             (try_begin), 
               (is_between, ":troop_level", 0, 14),
                              (party_stack_get_size,":num_troops",":party_no",":troop_iterator"),
                              (party_add_members,"p_temp_party",":cur_troop_id",":num_troops"),
                            #  (party_remove_members,":party_no",":cur_troop_id",":num_troops"),
             (else_try),  
                 (is_between, ":troop_level", 14, 24),
                              (party_stack_get_size,":num_troops",":party_no",":troop_iterator"),
                              (party_add_members,"p_temp_party_2",":cur_troop_id",":num_troops"),        
                             # (party_remove_members,":party_no",":cur_troop_id",":num_troops"),                              
             (else_try),  
              ...
              (is_between, ":troop_level", 24, 30),
                              (party_stack_get_size,":num_troops",":party_no",":troop_iterator"),
                              (party_add_members,"p_temp_party_3" ,":cur_troop_id",":num_troops"),
                            #   (party_remove_members,":party_no",":cur_troop_id",":num_troops"),
             (else_try),  
                              (party_stack_get_size,":num_troops",":party_no",":troop_iterator"),
                              (party_add_members,"p_temp_party_4",":cur_troop_id",":num_troops"),
                            #   (party_remove_members,":party_no",":cur_troop_id",":num_troops"),
         ...
         (party_clear, ":party_no"),
          ...
          (party_get_num_companion_stacks,":num_stacks","p_temp_party_4"),
         (try_for_range, ":troop_iterator", 0, ":num_stacks"),
           (party_stack_get_troop_id, ":cur_troop_id", "p_temp_party_4", ":troop_iterator"),
           (ge,":cur_troop_id",0),
           (party_stack_get_size,":num_troops","p_temp_party_4",":troop_iterator"),
           (party_add_members,":party_no",":cur_troop_id",":num_troops"),
         (try_end),
         
         (party_get_num_companion_stacks,":num_stacks","p_temp_party_3"),
         (try_for_range, ":troop_iterator", 0, ":num_stacks"),
           (party_stack_get_troop_id, ":cur_troop_id", "p_temp_party_3", ":troop_iterator"),
           (ge,":cur_troop_id",0),
           (party_stack_get_size,":num_troops","p_temp_party_3",":troop_iterator"),
           (party_add_members,":party_no",":cur_troop_id",":num_troops"),
         (try_end),
        ...
        # prisoners 
          (party_get_num_companion_stacks,":num_stacks","p_temp_party_5"),
         (try_for_range, ":troop_iterator", 0, ":num_stacks"),
           (party_stack_get_troop_id, ":cur_troop_id", "p_temp_party_5", ":troop_iterator"),
           (ge,":cur_troop_id",0),
           (party_stack_get_size,":num_troops","p_temp_party_5",":troop_iterator"),
           (party_add_prisoners,":party_no",":cur_troop_id",":num_troops"),
         (try_end),         
...
# Walled centres
       ...
       (try_for_range, ":party_no", walled_centers_begin, walled_centers_end),
         # don't re-sort troop configurations in player's centres
         (assign, ":result", 1),
         (try_begin), 
    ...
    (0.5,    
     [
        ...
        (try_begin),
            ...
            (try_end),
            (neq, ":faction", "fac_dark_knights"), # Lav fix 0.608b6: prevent indictments and treason among Dark Knights
...
#      (call_script, "script_center_change_trade_good_production", ":village_no", "itm_food_beef", ":production", 0),
    (try_end),
    ]),
...
#    ]),
  ...
  # Only if the player is male -- female characters will be told that they should seek out a faction through NPCs, possibly
   (32,
   [
     # NE 600 series - do not invite a player when in brutality mode
     ...
     (else_try),
         ...
         (try_begin),           
           ...
           # NE 
           (try_begin), 
           (eq, "$g_invite_faction", "fac_kingdom_1"), 
           (jump_to_menu, "mnu_invite_player_to_faction_swa"),
           (else_try),
         ...
         (else_try),
           ...
           (try_begin), 
           (eq, "$g_invite_faction", "fac_kingdom_1"), 
           (jump_to_menu, "mnu_invite_player_to_faction_without_center_swa"),
           (else_try),
  ...
  (8,
   [
       (try_for_parties, ":party_no"),
         ...
         # NE building 
           (try_begin), #Reroute caravans away from towns that ended up under siege - Jinnai
           (neg|party_is_in_any_town, ":party_no"),
           (party_get_slot, ":cur_ai_object", ":party_no", slot_party_ai_object),
           (neg|party_slot_eq, ":cur_ai_object", slot_center_is_besieged_by, -1),
           (store_faction_of_party, ":merchant_faction", ":party_no"),
           (party_get_slot, ":home", ":party_no", slot_party_home_center),
           (is_between, ":home", towns_begin, towns_end),
           (call_script, "script_cf_select_random_town_at_peace_with_faction_in_trade_route", ":home", ":merchant_faction"),
         ...
         (else_try),
           ...
           # NE building
           ## Add troops for caravan escort to existing caravans - Jinnai
           (try_begin),
             (party_get_num_companion_stacks, ":num_stacks", ":party_no"),
             (lt,":num_stacks",3),
             (party_slot_eq, ":cur_center", slot_center_has_caravan_escort, 1),
             (store_random_in_range,":rand",8,11),
             (party_add_members, ":party_no", "trp_mercenary_crossbowman", ":rand"),
             (store_random_in_range,":rand",8,11),
             (party_add_members, ":party_no", "trp_female_archer", ":rand"),
             (store_random_in_range,":rand",8,11),
             (party_add_members, ":party_no", "trp_mercenary_swordsman", ":rand"),
             (store_random_in_range,":rand",3,9),
             (party_add_members, ":party_no", "trp_mercenary_equite", ":rand"),
             (store_random_in_range,":rand",3,9),
             (party_add_members, ":party_no", "trp_female_shield_sister", ":rand"),
#updating for new troop names in NE 600 series - soulmata
          #   (store_random_in_range,":rand",8,11),
          #   (party_add_members, ":party_no", "trp_mercenary_crossbowman", ":rand"),
          #   (store_random_in_range,":rand",8,11),
          #   (party_add_members, ":party_no", "trp_mercenary_archer", ":rand"),
          #   (store_random_in_range,":rand",8,11),
          #   (party_add_members, ":party_no", "trp_mercenary_sergeant", ":rand"),
           (try_end),
           # Ne end building
           ...
           (try_begin),
             ...
             (call_script, "script_cf_do_merchant_town_trade", ":party_no", ":cur_center"),
           (try_end),
   ...
   [
    ...
    (try_begin),
      ...
      (try_for_range, ":cur_edible", food_begin, food_end),      
        (call_script, "script_cf_player_has_edible_food_of_type", ":cur_edible"),
    ...
    (try_for_range, ":unused", 0, ":consumption_amount"),
      ...
      (try_for_range, ":cur_food", food_begin, food_end),
        ...
        (call_script, "script_cf_player_has_edible_food_of_type", ":cur_food"),
  ...
  (24,
   [
     ...
     (try_for_range, ":i_slot", 0, ":inv_size"),
       ...
       (this_or_next|eq, ":item_id", "itm_food_beef"),
       (this_or_next|eq, ":item_id", "itm_food_chicken"),
        (eq, ":item_id", "itm_food_pork"),
  ...
  (0,
   [(troop_get_inventory_slot, ":cur_horse", "trp_player", 8), #horse slot
    ...
    (try_begin),
      ...
      (else_try),
        (assign, ":new_icon", "icon_player_female"),
      (try_end),
    (else_try),
 ...
 #Update how good a target player is for bandits
  ...
  (72,
   [
     ...
     (try_end),
     # NE noble
         (try_for_range, ":town_no", towns_begin, towns_end), # Recruit nobles from towns - Jinnai
      (call_script, "script_update_volunteer_troops_in_town", ":town_no"),
      (call_script, "script_update_npc_volunteer_troops_in_town", ":town_no"),
    (try_end),
  ...
  (12,
   [(try_for_range, ":center_no", centers_begin, centers_end),
      ...
      # ne building
      (try_begin),
        (eq,":cur_improvement",slot_center_university_placeholder),
        (faction_set_slot,"fac_player_supporters_faction",slot_faction_university,":center_no"),
      (else_try),
        (party_set_slot, ":center_no", ":cur_improvement", 1),
      (try_end),
      (try_begin),
        (eq,":cur_improvement",slot_center_has_cathedral),
        (faction_set_slot,"fac_kingdom_1",slot_faction_special_building_b,":center_no"),
      (else_try),
        (eq,":cur_improvement",slot_center_has_ivory_temple),
        (faction_set_slot,"fac_kingdom_2",slot_faction_special_building_b,":center_no"),
      (else_try),
        (eq,":cur_improvement",slot_center_has_palace),
        (faction_set_slot,"fac_kingdom_3",slot_faction_special_building_b,":center_no"),
      (else_try),
        (eq,":cur_improvement",slot_center_has_hall),
        (faction_set_slot,"fac_kingdom_4",slot_faction_special_building_b,":center_no"),
      (else_try),
        (eq,":cur_improvement",slot_center_has_chambers),
        (faction_set_slot,"fac_kingdom_5",slot_faction_special_building_b,":center_no"),
      (else_try),
        (eq,":cur_improvement",slot_center_has_baths),
        (faction_set_slot,"fac_kingdom_6",slot_faction_special_building_b,":center_no"),
      (try_end),
      ...
      (try_begin),
        # NE building
        (store_faction_of_party, ":faction", ":center_no"), # Letting the player see completed buildings - Jinnai
        (this_or_next|eq,":faction","fac_player_supporters_faction"),
        # ne building end
        (party_slot_eq, ":center_no", slot_town_lord, "trp_player"),
        (str_store_party_name_link, s4, ":center_no"),
        (display_log_message, "@Building of {s50} in {s4} has been completed."),
      (try_end),
      (try_begin),
        (is_between, ":center_no", villages_begin, villages_end),
  ...
  # Checking if a wound has healed - Jinnai
  (1,
   [(store_current_hours, ":cur_hours"),
    #(try_for_range, ":troop_no", active_npcs_begin, active_npcs_end),
    #  (troop_get_type,reg4,":troop_no"),
    #  (str_store_troop_name,s4,":troop_no"),
    #  (try_begin),
    #    (troop_get_slot, ":wound", ":troop_no", slot_troop_wound_str),
    #    (gt, ":wound", 0),
    #    (ge, ":cur_hours", ":wound"),
    #    (troop_set_slot, ":troop_no", slot_troop_wound_str, 0),
    #    (troop_raise_attribute, ":troop_no", ca_strength, 2),
    #    (display_message,"@{s4} has recovered from the wound affecting {reg4?her:his} strength."),
    #  (try_end),
    #  (try_begin),
    #    (troop_get_slot, ":wound", ":troop_no", slot_troop_wound_agi),
    #    (gt, ":wound", 0),
    #    (ge, ":cur_hours", ":wound"),
    #    (troop_set_slot, ":troop_no", slot_troop_wound_agi, 0),
    #    (troop_raise_attribute, ":troop_no", ca_agility, 2),
    #    (display_message,"@{s4} has recovered from the wound affecting {reg4?her:his} agility."),
    #  (try_end),
    #  (try_begin),
    #    (troop_get_slot, ":wound", ":troop_no", slot_troop_wound_cha),
    #    (gt, ":wound", 0),
    #    (ge, ":cur_hours", ":wound"),
    #    (troop_set_slot, ":troop_no", slot_troop_wound_cha, 0),
    #    (troop_raise_attribute, ":troop_no", ca_charisma, 2),
    #    (display_message,"@{s4} has recovered from the wound affecting {reg4?her:his} charisma."),
    #  (try_end),
    #  (try_begin),
    #    (troop_get_slot, ":wound", ":troop_no", slot_troop_wound_int),
    #    (gt, ":wound", 0),
    #    (ge, ":cur_hours", ":wound"),
    #    (troop_set_slot, ":troop_no", slot_troop_wound_int, 0),
    #    (troop_raise_attribute, ":troop_no", ca_intelligence, 2),
    #    (display_message,"@{s4} has recovered from the wound affecting {reg4?her:his} intelligence."),
    #  (try_end),
    #(try_end),
    (store_add, ":wound_severity", 1, "$setting_brutality_mode"),
    (try_for_range, ":troop_no", companions_begin, companions_end),
      ...
      (try_begin),
        ...
        (troop_raise_attribute, ":troop_no", ca_strength, ":wound_severity"),
        (display_message,"@{s4} has recovered from the wound affecting {reg4?her:his} strength."),
      (try_end),
      (try_begin),
        (troop_get_slot, ":wound", ":troop_no", slot_troop_wound_agi),
        ...
        (troop_set_slot, ":troop_no", slot_troop_wound_agi, 0),
        (eq, "$setting_use_wounding", 1),
        (troop_raise_attribute, ":troop_no", ca_agility, ":wound_severity"),
        (display_message,"@{s4} has recovered from the wound affecting {reg4?her:his} agility."),
      (try_end),
      (try_begin),
        (troop_get_slot, ":wound", ":troop_no", slot_troop_wound_cha),
        ...
        (troop_set_slot, ":troop_no", slot_troop_wound_cha, 0),
        (eq, "$setting_use_wounding", 1),
        (troop_raise_attribute, ":troop_no", ca_charisma, ":wound_severity"),
        (display_message,"@{s4} has recovered from the wound affecting {reg4?her:his} charisma."),
      (try_end),
      (try_begin),
        (troop_get_slot, ":wound", ":troop_no", slot_troop_wound_int),
        ...
        (troop_set_slot, ":troop_no", slot_troop_wound_int, 0),
        (eq, "$setting_use_wounding", 1),
        (troop_raise_attribute, ":troop_no", ca_intelligence, ":wound_severity"),
        (display_message,"@{s4} has recovered from the wound affecting {reg4?her:his} intelligence."),
      (try_end),
    (try_end),
    (try_begin),
        ...
        (gt, ":wound", 0),
        (ge, ":cur_hours", ":wound"),
        (troop_set_slot, "trp_player", slot_troop_wound_str, 0),
        (eq, "$setting_use_wounding", 1),
        (troop_raise_attribute, "trp_player", ca_strength, ":wound_severity"),
        (display_message,"@You have recovered from the wound affecting your strength.",0x20ff20),
    (try_end),
    (try_begin),
        (troop_get_slot, ":wound", "trp_player", slot_troop_wound_agi),
        (gt, ":wound", 0),
        (ge, ":cur_hours", ":wound"),
        (troop_set_slot, "trp_player", slot_troop_wound_agi, 0),
        (eq, "$setting_use_wounding", 1),
        (troop_raise_attribute, "trp_player", ca_agility, ":wound_severity"),
        (display_message,"@You have recovered from the wound affecting your agility.",0x20ff20),
    (try_end),
    (try_begin),
        (troop_get_slot, ":wound", "trp_player", slot_troop_wound_cha),
        (gt, ":wound", 0),
        (ge, ":cur_hours", ":wound"),
        (troop_set_slot, "trp_player", slot_troop_wound_cha, 0),
        (eq, "$setting_use_wounding", 1),
        (troop_raise_attribute, "trp_player", ca_charisma, ":wound_severity"),
        (display_message,"@You have recovered from the wound affecting your charisma.",0x20ff20),
    (try_end),
    (try_begin),
        (troop_get_slot, ":wound", "trp_player", slot_troop_wound_int),
        (gt, ":wound", 0),
        (ge, ":cur_hours", ":wound"),
        (troop_set_slot, "trp_player", slot_troop_wound_int, 0),
        (eq, "$setting_use_wounding", 1),
        (troop_raise_attribute, "trp_player", ca_intelligence, ":wound_severity"),
        (display_message,"@You have recovered from the wound affecting your intelligence.",0x20ff20),
    (try_end),
    ]),    
   ...
   [(assign, ":num_active_tournaments", 0),
    ...
    (try_for_range, ":center_no", centers_begin, centers_end),
      ...
      (try_begin),
        ...
        # NE 600 series - allow a trained militia a chance to fight off bandits
        # Disabled by Lav for 0.610: night bandits can appear no 
        #(try_begin),
        #  (str_store_party_name, s1, ":center_no"),
        #  (eq, ":continue", 1),
        #  (party_get_slot, ":militia_completed", ":center_no", slot_militia_quest_completed),
        #  # Skip the check entirely if they have no militia
        #  (gt, ":militia_completed", 0),
        #  (store_random_in_range, ":random_no", 0, 100),
        #  (store_random_in_range, ":militia_destroyed", 0, 100),
        #                
        #  (try_begin),
        #    #Bandits defeated militia - 20% chance
        #    (ge, ":random_no", 80),
        #    (assign, ":continue", 1),
        #    (call_script, "script_ne_disable_militia_on_village", ":center_no"),
        #  (else_try),
        #    #Bandits were driven off by militia
        #    (assign, ":continue", 0),
        #    (try_begin),
        #      #Militia was destroyed
        #      (le, ":militia_destroyed", 10),
        #      (call_script, "script_ne_disable_militia_on_village", ":center_no"),
        #    (else_try),
        #       #Militia survived
        #    (try_end),
      ...
      (else_try),
        (try_begin),
          ...
          (try_begin),
            (party_slot_eq, ":center_no", slot_party_type, spt_town),
            (val_div, ":random_chance", 2), # It is more difficult to find a few bandits in town
            # Need to add building effects
          (else_try),
            (party_slot_ge, ":center_no", slot_militia_quest_completed, 1),
            (val_mul, ":random_chance", 2), # In a village with militia a few night bandits will rarely stay for more than a day
          (try_end),
          (store_random_in_range, ":random_no", 0, 100),
...
# NE building and lords
  # lord asking the player for land and making sure the player's kingdom has a capital - Jinnai
  (6,
   [
    (faction_slot_eq, "fac_player_supporters_faction", slot_faction_state, sfs_active),
    (faction_slot_eq, "fac_player_supporters_faction", slot_faction_leader, "trp_player"),
    ...
    (assign, ":towns_owned", 0),
    (try_for_range, ":center_no", towns_begin, towns_end),
      (party_get_slot, ":centerlord", ":center_no", slot_town_lord),
      (try_begin),
        (eq, ":centerlord", "trp_player"),
        (val_add,":towns_owned",1),
      (else_try),
        (is_between,":centerlord",companions_begin,companions_end),
        (val_add,":towns_owned",1),
      (try_end),
    (try_end),
    (eq,"$setting_lords_ask",1),
    (try_for_range, ":center_no", castles_begin, castles_end),
      (party_get_slot, ":centerlord", ":center_no", slot_town_lord),
      (this_or_next|eq, ":centerlord", "trp_player"),
      (is_between,":centerlord",companions_begin,companions_end),
      (val_add,":castles_owned",1),
    (try_end),
    (this_or_next|gt,":towns_owned",1),
    (gt,":castles_owned",1),
    (assign, ":done", 0),
    (try_for_range, ":cur_troop", active_npcs_begin, active_npcs_end),
      ...
      (assign, ":towns_owned", 0),
      (try_for_range, ":center_no", walled_centers_begin, walled_centers_end),
        (party_slot_eq, ":center_no", slot_town_lord, ":cur_troop"),
        (val_add,":towns_owned",1),
      (try_end),
      (eq,":towns_owned",0),
      (store_random_in_range,":rand",1,101),
      (le,":rand",20),
      (assign,"$castle_meeting_selected_troop",":cur_troop"),
      (troop_get_slot,":renown",":cur_troop", slot_troop_renown),
      (try_begin),
        (gt,":renown",700),
        (assign,"$lord_request_land",2),
      (else_try),
        (assign,"$lord_request_land",1),
      (try_end),
...
#    (ge, "$g_last_rest_center", 0),
    ...
    (try_begin),
      ...
      (call_script, "script_adjust_finances", slot_finance_wages, ":total_cost"),
      # NE end finances
    (else_try),
      (gt, ":gold", 0),
      ...
      # NE finances
      (val_mul,":gold",-1),
      (call_script, "script_adjust_finances", slot_finance_wages, ":gold"),
      # NE end finances
    (try_end),
    ]),
...
## Give morale boost for festival square resting - Jinnai
  (1,
   [(neg|map_free),
    (is_currently_night),
    (ge, "$g_last_rest_center", 0),
    (party_slot_eq, "$g_last_rest_center", slot_town_lord, "trp_player"),
    (party_slot_eq, "$g_last_rest_center", slot_center_has_festival_square, 1),
    (store_current_hours, ":cur_hours"),
    (ge, ":cur_hours", "$g_last_rest_festival_until"),
    (store_add, "$g_last_rest_festival_until", ":cur_hours", 24),
    (party_get_morale, ":cur_morale", "p_main_party"),
    (store_add, ":new_morale", ":cur_morale", 5),
    (val_clamp, ":new_morale", 0, 100),
    (party_set_morale, "p_main_party", ":new_morale"),
    (display_message,"@Your men have enjoyed a day of festivities and relaxation in the town."),
    ]),
...
##      (try_end),
      ...
      (else_try),
        ...
        (try_for_parties, ":cur_party"),
          (store_faction_of_party, ":party_faction", ":cur_party"),
          (eq, ":party_faction", ":cur_kingdom"), # If different kingdom, do nothing
          ...
          (else_try),
            (party_set_faction, ":cur_party", "fac_outlaws"),
          (try_end),
        (try_end),
...
# Report to army quest 
  ...
  (24,
   [
         ...
         #Staples
      (item_set_slot, "itm_food_bread", slot_item_food_bonus, 8), #brought up from 4
      (item_set_slot, "itm_food_grain", slot_item_food_bonus, 2), #new - can be boiled as porridge
         
         #Fat sources - preserved
      (item_set_slot, "itm_food_fish_smoked", slot_item_food_bonus, 4),
      (item_set_slot, "itm_food_meat_dry", slot_item_food_bonus, 5),
      (item_set_slot, "itm_food_cheese", slot_item_food_bonus, 5),
      (item_set_slot, "itm_food_sausages", slot_item_food_bonus, 5),
      (item_set_slot, "itm_food_butter", slot_item_food_bonus, 4), #brought down from 8

         #Fat sources - perishable
      (item_set_slot, "itm_food_chicken", slot_item_food_bonus, 8), #brought up from 7
      (item_set_slot, "itm_food_beef", slot_item_food_bonus, 7), #brought down from 7
      (item_set_slot, "itm_food_pork", slot_item_food_bonus, 6), #brought down from 6
         
         #Produce
      (item_set_slot, "itm_food_olives", slot_item_food_bonus, 1),
      (item_set_slot, "itm_food_cabbages", slot_item_food_bonus, 2),
      (item_set_slot, "itm_food_grapes", slot_item_food_bonus, 3),
      (item_set_slot, "itm_food_apples", slot_item_food_bonus, 4), #brought down from 5

         #Sweet items
      (item_set_slot, "itm_food_dates", slot_item_food_bonus, 4), #brought down from 8
      (item_set_slot, "itm_food_honey", slot_item_food_bonus, 6), #brought down from 12
      
      (item_set_slot, "itm_drink_wine", slot_item_food_bonus, 5),
      (item_set_slot, "itm_drink_ale", slot_item_food_bonus, 4),
   ]),
...
# Train peasants against bandits
  (1,
   [
     ...
     # NE  600 Series - change from 20 base hours to 10 base hours to make this quest faster
     (store_sub, ":needed_hours", 14, ":trainer_skill"),
     (try_begin),
       (le, ":needed_hours", 3), # Let's make it 4 minimum base hours
       (assign, ":needed_hours", 4),
     (try_end),
     # End NE 600 Series
...
# Collect taxes
  (1, [(neg|map_free),
       ...
       (else_try),
         ...
         # NE finances
         (call_script, "script_adjust_finances", slot_finance_quest_rewards, ":collected_tax"),
         # NE end finances
       (try_end),
       (try_begin),
...
## Give mill and horse ranch prosperity every 2 weeks -- Jinnai
  (15 * 24,
    [(try_for_range, ":center_no", villages_begin, villages_end),
      (try_begin),
        (party_slot_eq, ":center_no", slot_center_has_fish_pond, 1), #Hmmmmm
        (call_script, "script_change_center_prosperity", ":center_no", 5),
      (try_end),
      (try_begin),
        (party_slot_eq, ":center_no", slot_center_has_horse_ranch, 1),
        (call_script, "script_change_center_prosperity", ":center_no", 2),
      (try_end),
    (try_end),
    ]),

## Give distillery prosperity and festival loyalty every month and move finances to previous month -- Jinnai
  (30 * 24,
    [(try_for_range, ":center_no", villages_begin, villages_end),
      (party_slot_eq, ":center_no", slot_center_has_distillery, 1),
      (call_script, "script_change_center_prosperity", ":center_no", 2),
    (try_end),
    (try_for_range, ":center_no", towns_begin, towns_end),
      (party_slot_eq, ":center_no", slot_center_has_festival_square, 1),
      (party_slot_eq, ":center_no", slot_town_lord, "trp_player"),
      (party_get_slot, ":player_relation", ":center_no", slot_center_player_relation),
      (val_add, ":player_relation", 3),
      (val_clamp, ":player_relation", -100, 100),
      (party_set_slot, ":center_no", slot_center_player_relation, ":player_relation"),
    (try_end),
    (try_for_range, ":idx", finances_begin, finances_end),
      (troop_get_slot, ":finance", "trp_finances_array_this", ":idx"),
      (troop_set_slot, "trp_finances_array_last", ":idx", ":finance"),
      (troop_set_slot, "trp_finances_array_this", ":idx", 0),
    (try_end),
    ]),
    
    
## Set a horse available for cheap purchase with a horse ranch - Jinnai
(72,
   [(try_for_range, ":center_no", villages_begin, villages_end),
      (party_slot_eq, ":center_no", slot_center_has_horse_ranch, 1),
      (party_get_slot, ":horses", ":center_no", slot_town_horse_available),
      (val_add, ":horses", 1),
      (val_clamp, ":horses", 0, 3),
      (party_set_slot, ":center_no", slot_town_horse_available, ":horses"),
    (try_end),
    ]),
   ...
   [(try_for_range, ":troop_no", active_npcs_begin, active_npcs_end),
      ...
      (try_begin),
        ...
        (try_begin),
          ...
          # NE building
          (try_begin), #Don't let them take university buffs with them - Jinnai
            (faction_slot_eq,"fac_player_supporters_faction", slot_faction_univ_pathfinding, 1),
            (troop_raise_skill,":troop_no",skl.pathfinding,-1),
          (try_end),
          (try_begin),
            (faction_slot_eq,"fac_player_supporters_faction", slot_faction_univ_spotting, 1),
            (troop_raise_skill,":troop_no",skl.spotting,-1),
          (try_end),
          (try_begin),
            (faction_slot_eq,"fac_player_supporters_faction", slot_faction_univ_surgery, 1),
            (troop_raise_skill,":troop_no",skl.surgery,-1),
          (try_end),
          (try_begin),
            (faction_slot_eq,"fac_player_supporters_faction", slot_faction_univ_wound, 1),
            (troop_raise_skill,":troop_no",skl.wound_treatment,-1),
          (try_end),
          (try_begin),
            (faction_slot_eq,"fac_player_supporters_faction", slot_faction_univ_engineering, 1),
            (troop_raise_skill,":troop_no",skl.engineer,-1),
          (try_end),
          # NE end building
...
## Make lords that switch to you not hostile to you begin - Jinnai  # pete - do we want this anymore? Comment for now

          (troop_get_slot, ":player_relation", ":troop_no", slot_troop_player_relation),
          (try_begin),
            (lt,":player_relation",0),
            (val_mul,":player_relation",-1),
            (call_script, "script_change_player_relation_with_troop", ":troop_no", ":player_relation"),
          (try_end),

# Make lords that switch to you not hostile to you end

          (try_begin), #Let them gain existing university buffs - Jinnai
            (faction_slot_eq,"fac_player_supporters_faction", slot_faction_univ_pathfinding, 1),
            (troop_raise_skill,":troop_no",skl.pathfinding,1),
          (try_end),
          (try_begin),
...
## Give the player a crown if they haven't been given one before
   (24,
    [
      (eq,"$has_crown",0),
      (faction_slot_eq, "fac_player_supporters_faction", slot_faction_state, sfs_active),
      ...
      # v585 Josef increase capacity to 2 and give 2 crowns
      (ge,":free_inventory",2),
      (assign,":give_crown",0),
      (try_for_range,":center_no",towns_begin,towns_end),
        (party_slot_eq, ":center_no", slot_town_lord, "trp_player"),
        (neg|party_slot_eq, ":center_no", slot_center_has_blacksmith, 0),
        (val_add,":give_crown",1),
        (assign,"$temp",":center_no"),
      (try_end),
      (gt,":give_crown",0),
      (assign,"$has_crown",1),
      (call_script, "script_add_notification_menu", "mnu_notification_give_crown", "$temp", 0),
   ]),
   ...
   (24,
    [
      ...
      (try_begin),
        ...
        (assign,":gift","itm_paladin_lance_2"),
      (else_try),
        (eq,"$players_kingdom","fac_kingdom_2"),
        (assign,":gift","itm_war_bow_ivory"), # itm_ivory_bow
      (else_try),
        (eq,"$players_kingdom","fac_kingdom_3"),
        (assign,":gift","itm_khergit_sword_two_handed_b"),
      (else_try),
        (eq,"$players_kingdom","fac_kingdom_4"),
        (assign,":gift","itm_nordic_axe_c"),
      (else_try),
        (eq,"$players_kingdom","fac_kingdom_5"),
        (assign,":gift","itm_halberd_b"),
      (else_try),
        (eq,"$players_kingdom","fac_kingdom_6"),
        (assign,":gift","itm_polehammer_b"),
      (try_end),
      (gt,":gift",0),
...
## University stuff - Jinnai
  (24 * 14,
    [
      (faction_slot_eq, "fac_player_supporters_faction", slot_faction_state, sfs_active),
      (faction_get_slot,":university","fac_player_supporters_faction",slot_faction_university),
      (gt,":university",0),
      (is_between, "$g_player_court", walled_centers_begin, walled_centers_end),
      (assign, ":capital", "$g_player_court"),
      (eq,":university",":capital"),
      (party_slot_eq,":capital",slot_town_lord,"trp_player"),
      (store_random_in_range,":rand",1,101),
      (le,":rand",90), # 10% chance of no breakthrough
      (assign,":upgrades",0),
      (try_for_range,":upgrade",university_upgrades_begin,university_upgrades_end),
        (faction_slot_eq,"fac_player_supporters_faction", ":upgrade", 0),
        (val_add,":upgrades",1),
      (try_end),
      (val_add,":upgrades",1),
      (store_random_in_range,":rand",1,":upgrades"),
      ...
      (try_for_range,":upgrade",university_upgrades_begin,university_upgrades_end),
        ...
        (val_add,":upgrades",1),
        (eq,":upgrades",":rand"),
        (assign,":result",":upgrade"),
      (try_end),
      (gt,":result",0),
      (faction_set_slot,"fac_player_supporters_faction",":result",1),
      (try_begin),
        (eq,":result",slot_faction_univ_pathfinding),
        (troop_raise_skill,"trp_player",skl.pathfinding,1),
        (try_for_range,":troop_no",heroes_begin, heroes_end),
          (store_troop_faction, ":faction", ":troop_no"),
          (eq,":faction","fac_player_supporters_faction"),
          (troop_raise_skill,":troop_no",skl.pathfinding,1),
        ...
        (try_for_range,":troop_no",companions_begin,companions_end),
          (troop_raise_skill,":troop_no",skl.pathfinding,1),
        (try_end),
        (str_store_string,s2,"@Pathfinding"),
        (str_store_string,s10,"@that it is possible to determine your location by studying the sun and the stars.\
 This will make travel for you and your lords faster and increase your Pathfinding skill by 1."),
      (else_try),
        (eq,":result",slot_faction_univ_spotting),
        (troop_raise_skill,"trp_player",skl.spotting,1),
        (try_for_range,":troop_no",heroes_begin, heroes_end),
          (store_troop_faction, ":faction", ":troop_no"),
          (eq,":faction","fac_player_supporters_faction"),
          (troop_raise_skill,":troop_no",skl.spotting,1),
        ...
        (str_store_string,s10,"@a means to make far objects appear closer using a tube and specially shaped glass. These\
 devices will be distributed to all your lords and increase your Spotting skill by 1."),
      (else_try),
        (eq,":result",slot_faction_univ_surgery),
        (troop_raise_skill,"trp_player",skl.surgery,1),
        (try_for_range,":troop_no",heroes_begin, heroes_end),
          (store_troop_faction, ":faction", ":troop_no"),
          (eq,":faction","fac_player_supporters_faction"),
          (troop_raise_skill,":troop_no",skl.surgery,1),
        ...
        (str_store_string,s10,"@that keeping an injury clean and changing the bandages regularly increases the chance of survival.\
 This will increase the Surgery skill for you and your lords by 1."),
      (else_try),
        (eq,":result",slot_faction_univ_wound),
        ...
        (str_store_string,s10,"@better methods of setting bones and suturing flesh, resulting in faster healing times for your troops.\
 The Wound Treatment skill for you and your lords has increased by 1."),
      (else_try),
        (eq,":result",slot_faction_univ_tactics),
        (str_store_string,s2,"@Tactics"),
        (str_store_string,s10,"@better methods of deploying troops, strategies to make use of terrain, and how to out maneuver the enemy.\
 All battles involving you or your lords will now have a Battle Advantage increase of 2."),
      (else_try),
        (eq,":result",slot_faction_univ_engineering),
        (troop_raise_skill,"trp_player",skl.engineer,1),
        (try_for_range,":troop_no",heroes_begin, heroes_end),
          (store_troop_faction, ":faction", ":troop_no"),
          (eq,":faction","fac_player_supporters_faction"),
          (troop_raise_skill,":troop_no",skl.engineer,1),
        ...
        (try_for_range,":troop_no",companions_begin,companions_end),
          (troop_raise_skill,":troop_no",skl.engineer,1),
        (try_end),
        (str_store_string,s2,"@Engineering"),
        (str_store_string,s10,"@new principles of engineering and better ways to contruct buildings. All of your lords have had their Engineer\
 skill increased by 1 and all royal engineers are now trained at a skill level of 5."),
      (else_try),
        (eq,":result",slot_faction_univ_belfry),
        (str_store_string,s2,"@Siege Towers"),
        (str_store_string,s10,"@a design for a better siege tower. Your siege towers will now move at twice their normal speed."),
      (else_try),
        (eq,":result",slot_faction_univ_repair),
        (str_store_string,s2,"@Metal Working"),
        (str_store_string,s10,"@a method of repairing bent and rusty metal without destroying its integrity. You can now repair damaged weapons\
 and armor at any blacksmith forge."),
      (try_end),
      (display_message,"@Your university has made a breakthrough in {s2}."),
      (call_script, "script_add_notification_menu", "mnu_notification_breakthrough", 0, 0),
   ]),

   
## Special building recruitment updates - Jinnai
  (24 * 7,
    [
      (try_for_range,":kingdom",kingdoms_begin,kingdoms_end),
        (neq,":kingdom","fac_dark_knights"),
        
        # v585 Josef this allows vassal special troops access - what could it affect?
...
#        (faction_get_slot,":troop3",":kingdom",slot_faction_special_troop_3),
        (faction_get_slot,":center",":kingdom",slot_faction_special_building_a),
        (store_random_in_range,":rand",1,4),
        (party_add_members,":center",":troop1",":rand"),
        (store_random_in_range,":rand",1,4),
        (party_add_members,":center",":troop2",":rand"),
#        (store_random_in_range,":rand",1,4),
#        (party_add_members,":center",":troop3",":rand"),
        ...
        (try_begin),
          ...
          (store_random_in_range,":rand",1,4),
          (party_add_members,":party",":troop1",":rand"),
          (store_random_in_range,":rand",1,4),
          (party_add_members,":party",":troop2",":rand"),
#          (store_random_in_range,":rand",1,4),
#          (party_add_members,":party",":troop3",":rand"),
        (else_try),
          (store_random_in_range,":rand",1,4),
          (faction_set_slot,":kingdom",slot_faction_special_troop_num_1a,":rand"),
          (store_random_in_range,":rand",1,4),
          (faction_set_slot,":kingdom",slot_faction_special_troop_num_2a,":rand"),
#          (store_random_in_range,":rand",1,4),
#          (faction_set_slot,":kingdom",slot_faction_special_troop_num_3a,":rand"),
        (try_end),
        (faction_get_slot,":center",":kingdom",slot_faction_special_building_b),
        (gt,":center",0),
        (store_random_in_range,":rand",1,4),
        (party_add_members,":center",":troop1",":rand"),
        (store_random_in_range,":rand",1,4),
...
#          (party_add_members,":party",":troop3",":rand"),
        (else_try),
          (store_random_in_range,":rand",1,4),
          (faction_set_slot,":kingdom",slot_faction_special_troop_num_1b,":rand"),
          (store_random_in_range,":rand",1,4),
          (faction_set_slot,":kingdom",slot_faction_special_troop_num_2b,":rand"),
#          (store_random_in_range,":rand",1,4),
#          (faction_set_slot,":kingdom",slot_faction_special_troop_num_3b,":rand"),
        (try_end),
      (try_end),
   ]),
...
#      (play_sound,"snd_quest_cancelled"),
     ...
     (store_random_in_range,":rand",0,5),
      ...
      (troop_set_name, "trp_dark_knight_lord",  "@Lady Larktin"),
      ...
      (troop_set_slot, "trp_dark_knight_lord", slot_troop_age, 30),    
      ...
      (call_script, "script_appoint_faction_marshall", "fac_dark_knights", "trp_dark_knight_lord"),   
...
# NE quests
   ...
   ## The player had no spare inventory capacity when he completed the Usurper quest
   ## so give him his reward now
   (24,
    [
      (eq,"$liege_reward",1),
      # (faction_slot_eq, "fac_player_supporters_faction", slot_faction_state, sfs_active),
...
# NE end quests
    ...
    # Dark Knights taking stance on faction issues much faster than regular lords (Lav fix for 0.608b6)
    (2, [
        (faction_slot_eq, "fac_dark_knights", slot_faction_state, sfs_active), # Dark Knights are active
        (faction_slot_ge, "fac_dark_knights", slot_faction_political_issue, 1), # Dark Knights have some issue on table
        (store_random_in_range, ":troop_no", "trp_dark_knight_lord1", "trp_kingdom_2_pretender"),
        (try_begin),
            (eq, ":troop_no", "trp_kingdom_1_pretender"),
            (assign, ":troop_no", "trp_dark_knight_lord"),
        (try_end),
        (troop_slot_eq, ":troop_no", slot_troop_occupation, slto_kingdom_hero),
        (troop_slot_eq, ":troop_no", slot_troop_stance_on_faction_issue, -1), # This knight has not yet made a decision
        (call_script, "script_npc_decision_checklist_take_stand_on_issue", ":troop_no"),
        (troop_set_slot, ":troop_no", slot_troop_stance_on_faction_issue, reg0),
    ]),

    # Caba'drin improved raids and sieges fix
    (0.25, [
        (ge,"$g_player_raiding_village",1),
    ...
    (0.25, [
        ...
        (try_begin),
            (ge, ":distance", raid_distance),
            (str_store_party_name_link, s1, "$g_player_besiege_town"),
            (display_message, "@You have broken off your siege of {s1}.", 0xff0000),
            (call_script, "script_lift_siege", "$g_player_besiege_town", 0),
            (assign, "$g_player_besiege_town", -1),
            (rest_for_hours, 0, 0, 0), #stop resting - abort
        (else_try),
            (ge, ":distance", raid_distance - 1),
            (map_free),
            (str_store_party_name_link, s1, "$g_player_besiege_town"),
            (display_message, "@You are too far from {s1}. Any further and your siege is broken!", 0xff4040),
        (else_try),
            (ge, ":distance", raid_distance - 2),
            (map_free),
            (str_store_party_name_link, s1, "$g_player_besiege_town"),
            (display_message, "@You are straying away from {s1}. Your siege lines are at risk.", 0xff8080),
        (else_try),
            (ge, ":distance", raid_distance - 3),
            (map_free),
            (str_store_party_name_link, s1, "$g_player_besiege_town"),
            (display_message, "@Do not stray too far away from {s1}, or your siege may break.", 0xffb0b0),
        (else_try),
            (store_current_hours, ":cur_hours"),
            (ge, ":cur_hours", "$g_siege_method_finish_hours"),
            (neg|is_currently_night),
  ...
  # NE 600 Series - Give villages a chance to raise their own militias
  (24,
    [
      (try_for_range, ":village_no", villages_begin, villages_end),
        (store_random_in_range, ":militia_trained", 0, 200),
        (eq, ":militia_trained", 1), # 0.5% chance per day to train a militia in a village spontaneously
        (call_script, "script_ne_enable_militia_on_village", ":village_no"),
      (try_end),
    ]),
  # End NE 600 Series - Give villages a chance to raise their own militias


    #(0, [
    #  (key_clicked, key_1),
    #  (map_free),
    #  (faction_set_slot, "fac_player_supporters_faction", slot_faction_civil_adviser, 0),
    #  (faction_set_slot, "fac_player_supporters_faction", slot_faction_logistics_adviser, 0),
    #  (faction_set_slot, "fac_player_supporters_faction", slot_faction_intel_adviser, 0),
    #  (faction_set_slot, "fac_player_supporters_faction", slot_faction_financial_adviser, 0),
    #  (display_message, "@All advisors fired!", 0xff0000),
      #(try_for_parties, reg1),
      #  (store_faction_of_party, reg2, reg1),
      #  (this_or_next|eq, reg2, "fac_player_faction"),
      #  (eq, reg2, "fac_no_faction"),
      #  (party_get_template_id, reg3, reg1),
      #  (str_store_party_name, s1, reg1),
      #  (str_store_faction_name, s2, reg2),
      #  (display_message, "@Found party {s1} ({reg1}) type {reg3} of {s2} ({reg2}).", 0xff0000),
      #(try_end),
    #]),
