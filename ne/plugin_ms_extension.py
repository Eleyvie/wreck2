...
def preprocess_entities(glob):
	# Retrieve item modifier names, price and rarity coefficients
	insert_init = WRECK.injections.setdefault('game_start_globals', [])
	modifiers = glob['item_modifiers']
	for index in xrange(len(modifiers)):
		str_id = ('item_modifier_0%d' if index < 10 else 'item_modifier_%d') % index
		imod_real_name = modifiers[index][1]
		str_offset = int(getattr(WRECK.s, str_id)) & 0xFFFFFFFF # Strip opmask
		glob['strings'][str_offset][1] = imod_real_name % '{s99}'
		insert_init.append((troop_set_slot, trp.static_data_array, 10 * index + _imod_offset_value, int(modifiers[index][2] * 1000000)))
		insert_init.append((troop_set_slot, trp.static_data_array, 10 * index + _imod_offset_rarity, int(modifiers[index][3] * 1000000)))
...
def debug_values(*argl):
	result = []
	debug = []
	for index in xrange(len(argl)):
		variable = argl[index]
		result.append((assign, reg(index + 80), variable))
		if type(variable) == str:
			debug.append(variable)
		elif isinstance(variable, VARIABLE):
			debug.append('%s = {reg%d}' % (variable.formatted_name(), index + 80))
		else:
			debug.append('V[%d] = {reg%d}' % (index, index + 80))
	result.append((display_message, "@DEBUG: %s." % ', '.join(debug)))
	return result

extend_syntax(debug_values)
