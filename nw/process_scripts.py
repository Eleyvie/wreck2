...
def save_scripts(variable_list,variable_uses,scripts,tag_uses,quick_strings):
  ...
  for i_script in xrange(len(scripts)):
    ...
    name = convert_to_identifier(func[0])
    if not name.startswith('game_'):
      if not name.startswith('cf_'):
        name = 'script_%d' % i_script
      else:
        name = 'cf_script_%d' % i_script
    if (type(func[1]) == list_type):
      file.write("%s -1\n"%(name))
      save_statement_block(file,name, 0,func[1], variable_list,variable_uses,tag_uses,quick_strings)
    else:
      file.write("%s %f\n"%(name, func[1]))
      save_statement_block(file,name, 0,func[2], variable_list,variable_uses,tag_uses,quick_strings)
    file.write("\n")
  file.close()
